...
export const CoordinatorErrorSchema = z.nativeEnum(CoordinatorError);

export const EIP712CoordinatorErrorSchema = z.nativeEnum(
  EIP712CoordinatorError
);

export const WalletErrorSchema = z.nativeEnum(WalletError);

export const AllowlistErrorSchema = z.nativeEnum(AllowlistError);

export const ERC20ErrorSchema = z.nativeEnum(ERC20Error);

export const RaiseIfInfernetErrorSchema = z
  .function()
  .args(HexSchema, z.instanceof(Subscription));
 ...
 */
export const raise_if_infernet_error = RaiseIfInfernetErrorSchema.implement(
  (encodedError, sub) => {
    const errors = {
    ...
    for (let i = 0; i < errorSelectors.length; i++) {
      ...
      // Compare the custom error byte selectors.
      if (encodedError === errorSelector) {
        if (
