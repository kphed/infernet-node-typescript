...
export const getAllCommaSeparatedPermutations =
  GetAllCommaSeparatedPermutationsSchema.implement((containers) => {
...
export const computePermutationHash = ComputePermutationHashSchema.implement(
  (permutation) =>
...
export class ContainerLookup {
  ...
  static methodSchemas = {
    get_containers: z.function().args(z.string()).returns(z.string().array()),
  };
  ...
  get_containers = ContainerLookup.methodSchemas.get_containers.implement(
    (hash) => this.#containerLookup[hash] ?? []
  );
}
